...
### Frontend (static/)
- **Form Management**: `job-form-*.js` (core, SSH, Restic, source-paths, rsyncd, globals)
- **Feature Modules**: `backup-browser.js`, `restore-*.js`, `config-*.js`, `job-inspect.js`
...
**Notifications**: `notifiers` library backend, Telegram/email, spam-prevention queuing with configurable intervals, batch message formatting, per-job integration with template variables, test capabilities, emoji-free
**UI**: Sectioned forms, real-time validation, share discovery, theming, password toggles, multi-path management, per-job inspection hubs (`/inspect?name=<jobname>`), source path validation buttons with [OK]/[WARN]/[ERROR] feedback
...
- DRY parsing with `JobFormParser.parse_source_configuration()`
...
- Modular single-responsibility files (`job-form-*.js`)
- AJAX with `FormData`, proper error handling with colored feedback ([OK]/[WARN]/[ERROR])
...
- **Template Variable Dependencies**: JavaScript files contain template variables (`{{SOURCE_PATHS_JSON}}`, `{{AVAILABLE_PROVIDERS_JSON}}`) that require server-side processing, breaking external file standards
...
**Modularization**: Proactively break up monolithic components. Single responsibility principle.
...
**Forms**: Multipart form data, dedicated parsers, real-time validation
...
**2025-08-16**: Container execution unified across all SSH Restic operations (init, backup, restore), comprehensive unit test coverage added, critical container command fix (removed duplicate `restic`), JavaScript extraction to external files, backup logging enhanced to show actual commands, command obfuscation utility created
